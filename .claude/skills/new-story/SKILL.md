...
7. `autoTrigger` — one of:
   ...
   - `{ kind: "on-shop-open" }` — auto-fires once when the player first lands on the `/shop` page (any shop). Wired in `ShopUI.tsx#useEffect`. Currently shop-id-agnostic — extend the variant with a `missionId` field if a specific shop needs its own briefing.
