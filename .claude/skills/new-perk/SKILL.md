---
name: new-perk
description: Scaffold a new mission-only perk — perks.ts entry, BootScene icon generator, HUD chip wiring, and (for actives) a switch case in CombatScene.useActivePerk.
---
...
6. **HUD chip rendering** — No React component to touch. The HUD perk chips are rendered inside Phaser by `CombatScene.refreshPerkChips()` (top-right of the combat canvas). It iterates `this.activePerks` and reads `PERKS[perkId]` for the icon/tint/name automatically — the new perk shows up with no extra wiring. For actives that show a charge counter, mirror the `perkId === "emp" ? this.empCharges : 1` line so the chip displays the right count.
...
- Active perks have a case in BOTH `applyPerk()` (charge gain) and `useActivePerk()` (charge consume).
