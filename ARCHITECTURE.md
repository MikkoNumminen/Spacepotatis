...
- **Leaderboard intro voice** ([src/game/audio/leaderboardAudio.ts](src/game/audio/leaderboardAudio.ts)) — single short voice clip ("Hall of Mediocrity") scheduled with a configurable lead-in delay before the leaderboard view fully resolves.
- **Master mute** — every engine registers with `setAllMuted(muted)` exported from `music.ts`. The `MuteToggle` button is the single switch. **Mute is session-only**: each cold load starts unmuted; toggling silences for the current session and persists across in-app navigation but not across reloads. There is no `localStorage` persistence — that design was dropped after stale `"spacepotatis:muted"` entries from prior testing sessions kept silencing the page on cold load with no obvious recovery. New audio engines must take their muted state from `setMuted()` calls only, never re-read `localStorage`.
