...
- **Recommended fix:** Two options — user picks at the gate:
  ...
  - **Phase 3 prerequisite:** before tightening, run `SELECT DISTINCT mission_id FROM spacepotatis.leaderboard WHERE mission_id NOT IN (<current MISSION_IDS>);` against prod (read-only, via a `scripts/check-schema.mjs`-style runner) to enumerate retired-but-leaderboard-populated ids. If the result is non-empty, surface those ids to the operator before the fix lands — they may want to (a) add them to `MISSION_IDS` to preserve read access, or (b) accept the data going dark. The check costs nothing and eliminates a "we didn't realize that mission id existed" surprise post-merge.
...
### SEC-004 — Error-message reflection inconsistency on `GET /api/save` + both `/api/handle` paths **Status:** fixed — pending-sha
...
- **Dependencies:** none.
- **Status:** fixed — commit af562f8, PR feat/security-sec-007-021-improve-restore-harness; regression test `scripts/improveRestoreHarness.test.mjs` (11 tests).
