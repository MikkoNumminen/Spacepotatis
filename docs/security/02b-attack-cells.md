...
- **Dependencies:** none.
- **Status:** fixed — pending-sha; PR feat/security-sec-025-026-client-save-hygiene; `void drainScoreQueue().then(...)` replaced with `await drainScoreQueue()` in `handleMissionComplete` in [src/components/GameCanvas.tsx](../../src/components/GameCanvas.tsx). Regression test [tests/security/saveScoreOrdering.test.ts](../../tests/security/saveScoreOrdering.test.ts) — 3 tests covering save-before-drain ordering, void-vs-await race documentation, and drain-result-before-status-update contract.
