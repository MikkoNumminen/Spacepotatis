...
- **Phase Security-Audit-Followups** — the 2026-05-04/05 `/security-audit` (Phase 0–2b) shipped 32 findings: 0 critical, 0 high, 9 medium, 15 low, 8 informational. Full plan + scenarios in [docs/security/02-findings-and-plan.md](docs/security/02-findings-and-plan.md) and [docs/security/02b-attack-cells.md](docs/security/02b-attack-cells.md). Wave 1 partially landed: SEC-001 (headers) #174, SEC-007+021 (improve-restore harness) #175, SEC-012 (AUTH_URL doc) #176, SEC-015 (actions SHA pin) #177. Remaining work below.
  - **Wave 1 remainder** (parallel, separate worktrees; unblock first):
    - **SEC-003** (Sonnet) — `?mission=` cast in `GET /api/leaderboard` + cache-key pollution. Tighten with `MissionIdSchema` enum; reject unknown ids 400. Run pre-flight `SELECT DISTINCT mission_id FROM spacepotatis.leaderboard WHERE mission_id NOT IN (...)` against prod first.
    - **SEC-011** (Sonnet, save-roundtrip-audit) — `seenStoryEntries` unbounded → audit-table storage DoS. Cap `z.array(z.string().max(64)).max(200)` AND truncate `save_audit.request_payload` > 64 KB to a `{truncated, size}` marker. Highest-impact fix in the audit.
    - **SEC-013** (Opus, save-roundtrip-audit) — TOCTOU on `prevRow` SELECT in `POST /api/save`. Wrap read+validate+upsert in `BEGIN; SELECT ... FOR UPDATE; ...; COMMIT;`. Multi-tab race overwrites with stale baseline; same incident class as the 2026-05-02 wipe.
    - **SEC-014** (Sonnet) — `ScorePayloadSchema.score` unbounded → trivial leaderboard #1 lock. Add per-mission `maxLegitScore(missionId)` derived from `waves.json` + `enemies.json` + 422 `score_implausible`; sanity Zod cap at 10M.
  - **Wave 2** (sequential, save-route region — touches `src/app/api/save/route.ts`, run save-roundtrip-audit before each):
    - **SEC-004** — drop `err.message` reflection from `GET /api/save` + both `/api/handle` paths; server still `console.error`s.
    - **SEC-005** — replace `session.user.email` with `playerId` in 4 save-rejection `console.warn` calls.
    - **SEC-016** — `LegacyShipSchema` `weaponLevels` / `weaponAugments` / `unlockedWeapons` `.max(50)` caps.
    - **SEC-017** (Opus) — credit-cap derivation uses user-submitted `completedMissions`; switch to `prevRow.completed_missions` + newly-grounded entries. Future-proof for any zero-prereq mission addition.
    - **SEC-020** — collapse 4 validator 422 codes to opaque `save_rejected` (preserve `save_regression` only — saveQueue depends on it).
    - **SEC-022** — `WeaponInventorySchema` `.max(50)`.
  - **Wave 3** (alone, architectural mini-design first): **SEC-002** rate limiting via Upstash Redis + `@upstash/ratelimit` (recommended). Per-route per-IP buckets: save 60/min, leaderboard 30/min, handle 10/min, GET leaderboard 120/min unauth.
  - **Wave 4** (sequential, auth surface, Opus):
    - **SEC-018** — rewrite `upsertPlayerId` as `INSERT ... ON CONFLICT DO UPDATE RETURNING id` (drops 500-on-first-concurrent-signin).
    - **SEC-019** — JWT callback rejects `email_verified === false`.
    - **SEC-008** — `next-auth` 5.0.0-beta.25 → latest beta hygiene bump.
  - **Wave 5** (parallel, doc/CI/UX):
    - **SEC-006** (Sonnet) — propose 90-day `save_audit` retention doc + commented-out cron. Activate when GH Actions audit-readiness cron opens the `save-architecture-ready` issue.
    - **SEC-010** — `docs/RIGHT_TO_ERASURE.md` runbook + `scripts/erase-player.mjs` using `dbWriteSafety.mjs` harness.
    - **SEC-023** — pass `--body-file` to `gh issue create` in `audit-readiness-check.yml` (drop shell interpolation).
    - **SEC-024** — `.husky/pre-commit` use `npx --no lint-staged`.
    - **SEC-025** — `src/game/state/sync.ts:244-246` strip raw payload from console.error on parse failure.
    - **SEC-026** — `useCloudSaveSync*` enforce `await saveNow()` then `await drainScoreQueue()` ordering.
    - **SEC-027** — `POST /api/save` reject `currentSolarSystemId` not in `unlockedSolarSystems`.
    - **SEC-028** — add `.github/dependabot.yml` (npm, weekly, group security).
    - **SEC-029** — add `permissions: contents: read` to `ci.yml`.
  - **Risk-accept (no fix, documented):** SEC-030 (leaderboard cache hit/miss timing), SEC-031 (leaderboard POST response-time oracle for own mission completion), SEC-032 (`timeSeconds` no minimum). Re-evaluate next audit.
  - **Audit infrastructure follow-ups:**
    - **SEC-012 operator action** — set `AUTH_URL` in Vercel env vars (Production AND Preview); confirm Google OAuth Console redirect-URI allow-list has only canonical URL. Doc-only fix landed; runtime pin still required.
    - **NSC-001** ([docs/security/04-other-findings.md](docs/security/04-other-findings.md)) — local-only branch `refactor/zod-content-accessors` adds runtime `Schema.parse(jsonData)` at module load — directly violates CLAUDE.md §5 (~98 kB first-load JS regression). Replace with the cast pattern + CI drift gate before merging, or abandon the branch.
    - **Phase 5 verification** (Opus) — re-derive dependency graph after Phase 3 module extraction; spot-check 3 modules for "could a fresh agent change this safely with only the docs?" Output: `docs/audit/05-final-report.md`.
    - **A-001 architectural decision** — pick rate-limit infra (Upstash Redis recommended) before Wave 3 dispatches. See SEC-002 trade-off table.
...
- ~~**Shop DETAILS voice — remaining items**~~ **— DONE.** PR #131 wired the convention `/audio/weapons/<id>-voice.mp3` / `/audio/augments/<id>-voice.mp3`; PR #133 shipped the three potato weapons, PR #149 the three pirate weapons, PR #152 all five augments. Future non-story voice work goes through the `/voice-asset` skill (script handoff, naming conventions, mono 24kHz 128kbps encoding).
