...
| Production-write safety helper | [scripts/_lib/dbWriteSafety.mjs](scripts/_lib/dbWriteSafety.mjs) (PR #102) — exports `parseFlags`, `writeBackup`, `requireConfirm` for any new prod-write script. Both `restore-player.mjs` and `improve-restore.mjs` use the full helper: `parseFlags` (dry-run-by-default + `--confirm`-to-mutate), `requireConfirm`, and `writeBackup`. `restore-player.mjs` additionally carries its own `--apply` / `--no-prompt` / `--i-have-printed-the-before-state` / `--force-overwrite-i-know-this-destroys-progress` flags layered on top. Backups land in `db-backups/` (gitignored). |
