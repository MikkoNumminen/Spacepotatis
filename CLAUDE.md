...
| [src/game/audio/](src/game/audio/)                        | Audio engines: `music.ts` (menu bed hot-swaps per solar system via `loadTrack` + combat bed + master-mute hub via `setAllMuted` — **mute is session-only, never read or write `localStorage["spacepotatis:muted"]` from new code**), `story.ts` (StoryModal cinematics; first-time-mode modals duck `menuMusic` while playing so the cinematic isn't competing with the galaxy bed), `storyLogAudio.ts` (Story-log bed), `menuBriefingAudio.ts` (landing-page voice queue), `itemSfx.ts` (per-category drop/shop cues — spawn-and-release per fire, no persistent template elements that count against iOS's ~6-element budget), `leaderboardAudio.ts` (Hall of Mediocrity intro voice), `sfx.ts` (procedural Web-Audio combat SFX — every `play*` call must wire `autoDispose(stopper, ...rest)` so nodes disconnect on `ended`, otherwise dead nodes accumulate for the lifetime of the AudioContext) |
