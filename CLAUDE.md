...
| Production-write safety helper | [scripts/_lib/dbWriteSafety.mjs](scripts/_lib/dbWriteSafety.mjs) (PR #102) — exports `parseFlags`, `writeBackup`, `requireConfirm` for any new prod-write script. Today only `writeBackup` is wired into `restore-player.mjs` and `improve-restore.mjs` (PR #103); those scripts use their own argv parsing because their flag surface predates the helper (`--apply`, `--player-email`, `--no-prompt`, `--i-have-printed-the-before-state`, `--force-overwrite-i-know-this-destroys-progress`). New scripts SHOULD adopt the full helper to inherit dry-run-by-default + `--confirm`-to-mutate. Backups land in `db-backups/` (gitignored). |
...
1. **Call `writeBackup()` before every UPDATE.** [scripts/_lib/dbWriteSafety.mjs](scripts/_lib/dbWriteSafety.mjs) exports `writeBackup({ prevRow, scriptName, flags })` which serializes the prevRow to `<repo>/db-backups/` (gitignored) before the destructive operation. Both shipped recovery scripts wire it (`restore-player.mjs` calls it inside the BEGIN/COMMIT transaction after the FOR-UPDATE read passes the shrink check; `improve-restore.mjs` calls it before its UPDATE). If `writeBackup` throws, ROLLBACK and refuse to mutate — the backup is the recoverability contract. The helper also exports `parseFlags` + `requireConfirm` (dry-run-by-default + `--confirm` gate) for new scripts; the existing recovery scripts predate that surface and use a hand-rolled `--apply` / `--player-email=<email>` / `--no-prompt` / `--i-have-printed-the-before-state` / `--force-overwrite-i-know-this-destroys-progress` flag set documented inline. New scripts SHOULD adopt `parseFlags` + `requireConfirm` so the dry-run default is automatic.
