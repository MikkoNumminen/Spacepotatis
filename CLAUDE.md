...
- **Game balance data lives in JSON** under [src/game/data/](src/game/data/). Never hard-code damage numbers, HP, spawn counts, etc. Read it through the typed accessors (`getEnemy`, `getWeapon`, `getMission`, etc.), not raw `as readonly XDefinition[]` casts at call sites. The accessors themselves do exactly one `as` cast at module load (no runtime Zod) — soundness is enforced by [src/game/data/__tests__/jsonSchemaValidation.test.ts](src/game/data/__tests__/jsonSchemaValidation.test.ts), which runs the matching `lib/schemas/*` parser against each JSON on every push. **Don't re-add `Schema.parse(jsonData)` at module load** — that's exactly what cost us ~98 kB on every static page's first-load JS before this pattern landed. CI is the drift gate now.
...
- **CI gates are blocking.** `typecheck`, `lint`, `test` (run WITH coverage), and `build` all run on every push and PR (see [.github/workflows/ci.yml](.github/workflows/ci.yml)). Suppressing errors (`@ts-expect-error`, `eslint-disable`) requires an inline justification comment AND a strong reason — the audit deleted two such suppressions because they were hiding a real bug.
- **Coverage is a ratchet gate.** The CI test step is `npm run coverage` (`vitest run --coverage`); the thresholds in [vitest.config.ts](vitest.config.ts) (`statements 84 / branches 76 / functions 83 / lines 87`, ~5 pts below the 2026-06-13 baseline) fail the build if coverage drops. When coverage rises durably, raise the floors to match — **never lower a floor to make a red PR green; add the missing test instead.** Genuinely-untestable-from-node files (WebGL/Phaser/React-hook surfaces) are handled by the `coverage.exclude` list, not by lowering thresholds.
...
npm test             # vitest run — 1064 tests
...
| JSON ↔ schema drift gate (CI test) | [src/game/data/__tests__/jsonSchemaValidation.test.ts](src/game/data/__tests__/jsonSchemaValidation.test.ts) — runs each `lib/schemas/*` against its `game/data/*.json` once per `npm test`. Data accessors do a single `as` cast with no runtime parse, so Zod stays out of every static page's first-load JS (~98 kB saving on landing/shop). When you add a new JSON-backed accessor, add a matching `it(...)` row here. |
...
| Server-side regression guard (no-shrink check on completedMissions / unlockedPlanets / playedTimeSeconds) | `validateNoRegression` in [src/lib/saveValidation.ts](src/lib/saveValidation.ts) — wired into `/api/save` POST (PR #94). Pairs with the client-side `isHydrationCompleted` gate in [src/game/state/syncCache.ts](src/game/state/syncCache.ts) so a load failure never POSTs INITIAL_STATE over a real save. A 422 here returns `error: "save_regression"` and is treated as TRANSIENT by the saveQueue (PR #96) — the snapshot is held until the next successful loadSave hydrates the local state and the new POST passes the guard. |
| Save durability queue (single-slot localStorage snapshot + account-stamped flush) | [src/game/state/saveQueue.ts](src/game/state/saveQueue.ts) — see ARCHITECTURE.md §4b. localStorage key is versioned (`spacepotatis:pendingSave:v2`); the `:v1` shape lacked a playerEmail stamp and could leak across accounts on shared browsers, so the read path silently drops any leftover `:v1` blob. Every pending save is stamped with `playerEmail` (PR #100) and the flush refuses to POST if the stamp doesn't match the currently signed-in account. Sign-out clears the queue via `clearLoadSaveCache` + the saveQueue's account-mismatch drop. |
| Save/load module-level cache (account email + hydration flag + last load result) | [src/game/state/syncCache.ts](src/game/state/syncCache.ts) — split out of sync.ts so landing-page bundles don't pull Zod for cache-only reads. Owns `currentPlayerEmail` (account-swap detection that resets `hydrationCompleted`), `lastLoadResult` (rich `LoadResult` mirror), and the `inflight` de-dup slot. |
| Structured load result | `LoadResult` union in [src/game/state/sync.ts](src/game/state/sync.ts) (PR #101): `server-loaded` / `anon` / `no-save` / `pending-only` / `load-failed`. Replaces the prior boolean return that collapsed "fresh account" and "we couldn't read the server" into a single `false` — the bug behind the silent INITIAL_STATE masquerade. |
| Save-load error UX | [src/components/SaveLoadErrorOverlay.tsx](src/components/SaveLoadErrorOverlay.tsx) + [src/components/hooks/useCloudSaveSyncLogic.ts](src/components/hooks/useCloudSaveSyncLogic.ts) (PR #101) — pure decision helpers (`loadResultToState`, `cachedResultToState`, `decideFetch`) that the React hook composes. Surfaces a full-screen overlay on `load-failed` instead of letting the splash clear over INITIAL_STATE. |
| Forensic save audit log | `spacepotatis.save_audit` table (migration [db/migrations/20260503000000_add_save_audit.sql](db/migrations/20260503000000_add_save_audit.sql)) + `writeSaveAudit` in [src/app/api/save/route.ts](src/app/api/save/route.ts) (PR #98). One row per authenticated POST `/api/save` attempt — success, validator rejection, or server error — capturing the request payload, prev snapshot, response status + error code, request IP, and user agent. Audit failure never blocks a save. |
| Production-write safety helper | [scripts/_lib/dbWriteSafety.mjs](scripts/_lib/dbWriteSafety.mjs) (PR #102) — exports `parseFlags`, `writeBackup`, `requireConfirm` for any new prod-write script. `improve-restore.mjs` uses the full helper (`parseFlags` for dry-run-by-default + `--confirm`-to-mutate, `requireConfirm` to gate, `writeBackup` before UPDATE). `restore-player.mjs` only uses `writeBackup` and carries its own hand-rolled `--apply` / `--player-email` / `--no-prompt` / `--i-have-printed-the-before-state` / `--force-overwrite-i-know-this-destroys-progress` flag set documented inline (predates the helper). Backups land in `db-backups/` (gitignored). |
...
1. **Call `writeBackup()` before every UPDATE.** [scripts/_lib/dbWriteSafety.mjs](scripts/_lib/dbWriteSafety.mjs) exports `writeBackup({ prevRow, scriptName, flags })` which serializes the prevRow to `<repo>/db-backups/` (gitignored) before the destructive operation. Both shipped recovery scripts wire it inside their respective transactions (`restore-player.mjs` after the FOR-UPDATE read passes the shrink check; `improve-restore.mjs` after the FOR-UPDATE read inside its BEGIN/COMMIT wrapper). If `writeBackup` throws, ROLLBACK and refuse to mutate — the backup is the recoverability contract. The helper also exports `parseFlags` + `requireConfirm` (dry-run-by-default + `--confirm` gate); `improve-restore.mjs` uses the full helper. `restore-player.mjs` predates the helper and uses its own hand-rolled `--apply` / `--player-email` / `--no-prompt` / `--i-have-printed-the-before-state` / `--force-overwrite-i-know-this-destroys-progress` flag set documented inline. New scripts SHOULD adopt `parseFlags` + `requireConfirm` so the dry-run default is automatic.
