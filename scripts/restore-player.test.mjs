...
const SCRIPT_PATH = path.join(HERE, "restore-player.mjs");

// Run the script as a subprocess with a hermetic env (no DATABASE_URL) so it
// physically cannot reach a real database. Returns { status, stdout, stderr }.
function runScript(args) {
  // Strip any inherited DATABASE_URL — these tests must never touch a real DB.
...
// End-to-end smoke tests for the orchestration layer (main()). The unit tests
// above pin the parser + diff helpers in isolation; these prove the SAFETY
// CONTRACT holds when the script is actually executed:
//   - bare args = dry-run, no DB writes
//   - --apply without --player-email = exit 2
//   - --apply with mismatched --player-email = exit 2
// The subprocess runs with no DATABASE_URL so it physically cannot reach a DB
// even if the orchestration regressed.
describe("restore-player main() safety contract (subprocess smoke)", () => {
  it("bare args do not issue any DB write (default-dry-run contract)", () => {
    const result = runScript(["fake@example.com"]);
    // Exit 0 (clean dry-run) or 1 (no DATABASE_URL — never reached the apply
    // branch) are both acceptable. Exit 2 = arg validation failure (wrong);
    // exit 3 = shrink refusal (wrong — we never hit DB).
    expect([0, 1]).toContain(result.status);
    const combined = `${result.stdout}\n${result.stderr}`;
    // No SQL UPDATE statement should appear anywhere — neither emitted to the
    // DB nor logged in stdout/stderr.
    expect(combined).not.toMatch(/UPDATE\s+spacepotatis/i);
    // The DRY-RUN marker proves we entered the safe branch (or at least never
    // entered the apply branch — main() prints `mode: DRY-RUN` before any DB
    // call when --apply is absent).
    expect(combined).toMatch(/DRY-RUN/);
  });

  it("--apply without --player-email exits 2 with no DB writes", () => {
    const result = runScript(["fake@example.com", "--apply"]);
    expect(result.status).toBe(2);
    const combined = `${result.stdout}\n${result.stderr}`;
    expect(combined).not.toMatch(/UPDATE\s+spacepotatis/i);
    expect(combined).toMatch(/--player-email/);
  });

  it("--apply with mismatched --player-email exits 2 with no DB writes", () => {
    const result = runScript([
      "fake@example.com",
      "--apply",
      "--player-email=mismatched@example.com",
    ]);
    expect(result.status).toBe(2);
    const combined = `${result.stdout}\n${result.stderr}`;
    expect(combined).not.toMatch(/UPDATE\s+spacepotatis/i);
    expect(combined).toMatch(/does not match/);
  });
});
