...
// INTERNAL to the `audio` module — the MusicEngine class is the machinery
// behind the menuMusic / combatMusic / shopMusic singletons in ./music.ts
// (the public surface). NOT re-exported from the audio barrel (index.ts):
// construct instances in music.ts; don't import the class elsewhere.
//
// One HTMLAudioElement per engine, manual loop with a fade-out → silence →
// fade-in seam so a long stay on one track sounds like the music takes a
// breath rather than restarting on a hard cut. Mute state is owned by
// AudioBus (the engine implements AudioBusEngine.setMuted and self-registers
// in the constructor). A self-healing watchdog + retry recover from autoplay
// blocks within ~2s.
//
// iOS Safari budget: the platform caps simultaneous HTMLAudioElement
// instances at ~6 per page. Engines lazy-create elements only when about to
// play and release them on ended/stop (src = "") so the slot frees
// immediately; only menuMusic (keepAlive) stays persistent.
...
// Exposed only via the `menuMusic` / `combatMusic` / `shopMusic` singletons
// in ./music.ts — not re-exported from the audio barrel.
