...
|---|---|---|
| [`GameCanvas.tsx`](./GameCanvas.tsx) | `/play` page | **GOD-FILE (338 LOC, partially split 2026-05-29)** — the main orchestrator. Mode machine + fade overlay + victory state machine now live in dedicated hooks (`useGameMode`, `useTransitionOverlay`, `useVictoryFlow`); story-trigger wiring already in `useStoryTriggers`; cloud-save sync in `useCloudSaveSync`. Remaining inline concerns: auto-warp logic, scene-select bridge, renderer error overlay, splash-step assembly. |
...
- **Adding a NEW stat row / shop upgrade is a multi-module change → use `/equipment`.** The `/equipment` skill owns the end-to-end flow (id union → content registry + curve → state field/mutator → this UI → the SAVE round-trip for a new upgrade level field). Don't reconstruct it from this README. The render row lives in `loadout/WeaponStatsView.tsx` (per-weapon stats) or a `<StatChip>` in `loadout/WeaponCard.tsx` (clickable ship-stat chips), but the VALUE must already exist in `content` — static fields in `src/game/data/*.json`, a derived curve in [`src/game/data/upgradeCurves.ts`](../game/data/upgradeCurves.ts), or a `ShipConfig` getter — never hard-coded in the component (CLAUDE.md §9). **Which DETAILS modal:** per-weapon stat → `WeaponDetailsModal`; hull/shield/reactor purchasable → `UpgradeDetailsModal`; aggregate clickable ship stat → `StatDetailsModal`.
