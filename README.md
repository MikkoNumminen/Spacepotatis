...
The April quality push above was a planned tidy-up. A few weeks later, an unplanned one happened: a real player's save game disappeared from the database. In the spirit of full disclosure: the "real player" was the developer's own account, which was lucky in the sense that the bug was extremely reproducible (you can just run the game) and unlucky in the sense that the developer was, you know, the person who would have to fix it.

Here is the post-mortem, because incidents are how you find out what your software actually does, as opposed to what you thought it did.

**What broke.** The game's save endpoint (`POST /api/save`) had three "anti-cheat" guards designed to reject suspicious saves: one for impossible playtime jumps, one for impossible credits jumps, and one to make sure the unlock chain was internally consistent. All three were checking that things hadn't grown too FAST. None of them noticed if something had grown SMALLER. So when a buggy client (later root-caused to a race between page-load and the first save-out) sent a payload with credits=0, completedMissions=[], and playedTimeSeconds=0, every guard said "looks fine, that's not inflation," and the server happily wrote zeros over the real save. The player's progress evaporated.

**The acute response, that same day.** Two new guards landed in production within hours of the diagnosis. The server got `validateNoRegression` — a check that refuses any save whose `completedMissions`, `unlockedPlanets`, or `playedTimeSeconds` would shrink. The client got `isHydrationCompleted` — a flag that prevents the game from posting a save before it has confirmed what's actually in the server. Either one alone would have prevented the incident; both together is belt-and-suspenders. The full play-by-play of what each of the nine emergency PRs did lives in [`docs/INCIDENT_RUNBOOK.md`](docs/INCIDENT_RUNBOOK.md).

**The recovery script that lost the data twice.** The developer wrote a small script (`scripts/restore-player.mjs`) to put the missing rows back. It worked perfectly. The developer then ran it a second time to double-check, and it lost everything the first run had just recovered (the script had hardcoded values, so re-running it stomped any progress earned since the first restore). So a follow-up PR taught the script to default to "dry-run" mode (it prints what it WOULD change but does nothing), require an explicit `--apply` flag, refuse to overwrite if any monotonic field would shrink, and credit-floors so it can never reduce a number even with `--force`. Now you have to type quite a lot of words, including the literal flag `--force-overwrite-i-know-this-destroys-progress`, to actually destroy progress. Naming flags after their consequences turns out to be a great deterrent.

**The deeper realization.** The two new server guards reduced the hit rate to roughly zero, but the data model itself still permits destruction. The `save_games` table is one row per player; every save POST overwrites it in place. Every guard is a band-aid on an architecture that lets a single bad write erase everything. The structural fix is to make the save table append-only — every POST INSERTs a new row, "current save" is `ORDER BY created_at DESC LIMIT 1`, and a wipe becomes an INSERT instead of a destructive UPDATE. Restore becomes `OFFSET 1 LIMIT 1`. Neat in shape. Six lines of SQL.

**The discipline part: not rushing the structural fix.** The temptation was to ship that migration the same week and call it a victory. The temptation was resisted. Database migrations on already-fragile production data deserve evidence, not vibes. So the next thing built was a `save_audit` table — a forensic recorder that captures every save POST: the request body, the previous server snapshot, the response code, and metadata like IP and user-agent. Every attempt — success, rejection, server error — is now an audit row. Future post-mortems start with a single SQL query instead of guesswork. Future architectural decisions start with a real distribution of payload sizes, error codes, and write frequency instead of guesses about same.

**The cron that announces when the data is ready.** Because the developer is too busy to remember to check the audit table, there is now a tiny GitHub Actions workflow ([`.github/workflows/audit-readiness-check.yml`](.github/workflows/audit-readiness-check.yml)) that runs once a day, queries the audit table, and quietly does nothing as long as there isn't enough data. The day all three thresholds pass — at least 100 audit rows, at least 2 distinct players, at least 3 days of history — the workflow opens a GitHub issue titled "save_audit ready — start Phase Save-Architecture" with the readiness report attached. That issue's appearance in the developer's notifications is the cue to start the structural fix. The work doesn't get forgotten, doesn't get rushed, and waits until the data justifies it.

**What this is supposed to demonstrate.** Three things. One: when prod breaks, ship the mitigation first and the post-mortem later — don't paralyze yourself reading logs while data continues to die. Two: there's a real difference between fixing a bug and fixing the bug class. The first is hours; the second is weeks; doing the second is what stops the same shape of incident from recurring six months later. Three: observability before architecture. Build the recorder, let it run, then design from what you actually see. Ten lines of cron beat ten thousand lines of speculation.
