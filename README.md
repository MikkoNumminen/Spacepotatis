...
## Why this exists (the not-so-secret agenda)

This is a **portfolio project**. The point is to show, with one shippable artifact, how Mikko builds software in 2026: agentic AI workflows on the editing side, self-built generative tools on the content side, and an honest production incident with a documented post-mortem. The game is the wrapper; the engineering is the product. If you're hiring, the three bits below are the tour.
...
**Estimated saving: ~2.76M tokens/year** across ~300 invocations — calculated by comparing a skill-guided run against a from-scratch grep-and-derive pass on the same task. The methodology and per-skill breakdown live in [docs/SKILLS.md](docs/SKILLS.md). The numbers are educated guesses; the architecture is the point.
...
Every note of music and every spoken line in the game is original, generated locally on Mikko's machine, and 100% copyright-free. No commercial voice actor, no licensed voice model, no stock library. Both pipelines live in their own repos:
...
- **Voice** → narrated by *Grandma*, one consistent character across menus, briefings, item-pickup cues, and a replayable Story log. Generated by [**AudiobookMaker**](https://github.com/MikkoNumminen/AudiobookMaker), Mikko's wrapper around the open-source Chatterbox text-to-speech model. Edit the script, regenerate in seconds, drop the file in.

The voice does double duty: audiobook-style narrative *and* gameplay feedback. *"You bought a new gun." "You cleared the system — there's more out there."* The line between story and tooltip is blurred on purpose. Music and voice both implement the boring-but-necessary stuff: autoplay-respecting first-input arming, ducking under combat SFX, fade-with-a-breath looping, session-only mute.
...
- **PostgreSQL on Neon, talked to via Kysely** — saves and leaderboard. No ORM (Object-Relational Mapper); we write SQL through a typed builder. Schema migrations are plain `.sql` files in `db/migrations/`.
...
- **Vercel Hobby** — the constraint that drives every architectural decision: client-side first, static-by-default, API routes on Edge runtime where possible.
