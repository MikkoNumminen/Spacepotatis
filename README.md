...
Spacepotatis tells a story as you play. At important moments — the very first time you launch the game, the moment a new mission becomes available, the aftermath of a tough boss — a popup fades up over the galaxy view, a quiet music bed comes in underneath, and a narrator reads you the next chapter of the tale. There's a real storyline running alongside the shooting, broken into bite-sized beats so each one fits before you fly off to the next thing.

Every storyline beat carries its own custom music, its own narration audio, and the same text on screen for anyone who's playing muted or who'd rather read along.

If you miss a beat — or you just want to hear it again — open the **user menu** in the top-right corner of the galaxy view and pick **Story log**. Every storyline entry you've already unlocked sits in there, ready to replay from the beginning, music and voice and all. Entries you haven't reached yet stay hidden so the path ahead doesn't spoil itself.

A short word on how the audio is put together, because it's a fun corner of recent tech.

The **music** for each story beat is written in Strudel, the same text-based music tool described in the Music section above. Same recipe: tap out a few lines of code, hear what you wrote, iterate, export the result as a regular audio file. The story beds are deliberately sparser than the menu track so the narration has room to breathe.

The **voiceover** is generated by Mikko's own **audiobookmaker** app — a small tool he built that feeds the story script through **Chatterbox**, an open-source text-to-speech model (a piece of software that turns written sentences into natural-sounding spoken audio). The whole pipeline runs locally on Mikko's machine; no commercial voice actor and no licensed voice model is involved, so every spoken line in the game is fully copyright-free. A nice practical side-effect is that dialogue becomes cheap to iterate — edit the script, regenerate the audio in seconds, drop the new file into the project. If a line doesn't land, it gets rewritten and re-spoken without anyone needing to book a recording session.
